<template>
    <v-card>
        <v-toolbar>
            <v-toolbar-title>Local Storage</v-toolbar-title>
            <v-spacer></v-spacer>
            <v-toolbar-items>
                <v-btn icon>
...
export default {
    ...
    computed: {
        testValue () {
            return this.$store.state.localStorage.testValue || 'Get or set to see current'
        },
        param () {
            return (key) => {
                return this.$store.getters[`localStorage/${key}`]
            }
        },
